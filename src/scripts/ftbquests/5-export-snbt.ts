...
function exportSNBT(): void {
  ...
  for (const [hashKey, viTextRaw] of translatedEntries.entries()) {
    ...
    const escapeSafe = (str: string): string => {
  ...
  for (const [relPath, keysToReplace] of Object.entries(fileData)) {
    ...
    content = content.replace(snbtRegex, (match: string, key: string, _oldVal: string) => {
      if (keysToReplace[key]) {
        replacedCount++;
        // FIX LỖI MẢNG: Đảm bảo định dạng mảng luôn cách Key một khoảng trắng cho SNBT dễ thở
        const newVal = keysToReplace[key];
        return `${key}: ${newVal}`;
      }
      return match;
    });
